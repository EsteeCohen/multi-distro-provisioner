#!/usr/bin/env bash
# setup_docker.sh -- installs Docker Engine from Docker's official repository.
#
# Closes #5
#
...
if is_installed docker; then
    ...
    case "${distro}" in
        ...
        ubuntu|debian)
            ...
            # ca-certificates -> allows curl to verify HTTPS certificates
            # curl            -> downloads files from the internet
            # gnupg           -> handles GPG key import
            ...
            # $(. /etc/os-release && echo "$VERSION_CODENAME") -> e.g. "jammy"
            ...
            # containerd.io          -> the container runtime docker-ce runs on top of
...
#
# /var/run/docker.sock is the Unix socket file -- the communication channel
...
#   This is why we only add trusted users (devops, appuser) -- not everyone.
...
for user in devops appuser; do
    if user_exists "${user}"; then
        ...
        log_warn "  User ${user} does not exist yet -- skipping"
    fi
done
