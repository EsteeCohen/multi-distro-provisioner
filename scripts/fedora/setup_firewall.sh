#!/usr/bin/env bash
# setup_firewall.sh — configures firewalld on Fedora.
#
# Closes #4
#
# WHY firewalld on Fedora?
#   Fedora, RHEL, and CentOS all use firewalld as the default firewall.
#   This is RHCSA exam material — you must know firewall-cmd.
...
#   The default zone is "public" — which assumes untrusted networks.
#   Rules you add go into a zone, not globally.
#
#   Common zones:
#     public  → untrusted network (internet-facing) — we use this
#     trusted → all connections accepted (LAN)
...
#     without --permanent → rule is active NOW but lost after reload/reboot
#     with --permanent    → rule is saved to disk and survives forever
